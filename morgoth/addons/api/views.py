...
class AddonGroupViewSet(ModelViewSet):
    ...
    def sync(self, request, *args, **kwargs):
        ...
        for group in groups:
            ...
            # Balrog doesn't accept periods in the alias so we will replace periods with hyphens
            rule_name = 'morgoth-sysaddons-ff{}-{}'.format(
                group.browser_version, channel).replace('.', '-')
            ...
            if not synced:
                ...
                try:
                    ...
                    balrog.request('releases', method='POST', data={
                        'name': group.name, **release_data})
                else:
                    release_data['data'] = release_data.pop('blob')
                    balrog.request('releases/{}'.format(group.name), method='POST',
                                   data=release_data)

                rule_data = {
                    'channel': balrog_channel,
                    'priority': 1000,
                    'backgroundRate': 100,
                    'product': 'SystemAddons',
                    'update_type': 'minor',
                    'version': '{}'.format(group.browser_version),
                    'mapping': group.name,
                    'comment': 'Generated by Morgoth.',
                }
                try:
                    balrog.request('rules/{}'.format(rule_name))
                except HTTPError:
                    balrog.request('rules', method='POST', data={
                        'alias': rule_name, **rule_data})
                else:
                    balrog.request('rules/{}'.format(rule_name), method='POST', data=rule_data)
